#!/bin/bash
# v2 architecture probes — 4 runs × 300k ≈ 40 min total
#
# Tests three targeted improvements vs the corrected tfm_base_300k baseline:
#   v2-D  mlp_proj=true      non-linear slot/state projections
#   v2-E  use_reactive=true  direct robot_state bypass path (latent=192d)
#   v2-F  attn_pool=true     attention-weighted slot pooling vs mean
#   v2-ALL all three + ffn_mult=4  full v2 architecture
#
# Run AFTER run_2h_sweep.sh completes (need tfm_base_300k corrected baseline).
# Baseline: MT=27.0mm  CI=5.0mm  F8=6.5mm
#
# Usage:
#   nohup bash run_v2_probes.sh &
#   tail -f results/v2_probes/probes.log

set -euo pipefail
source .venv/bin/activate

OUTROOT="results/v2_probes"
...
run_one() {
    ...
    local evaldir="results/eval/v2_probes/$name"
...
echo "════════════════════════════════════════════════════════" | tee -a "$LOG"
echo " v2 architecture probes — $(date)" | tee -a "$LOG"
...
echo "" | tee -a "$LOG"
echo "════════════════════════════════════════════════════════" | tee -a "$LOG"
echo " V2 PROBES COMPLETE — $(date)" | tee -a "$LOG"
echo "════════════════════════════════════════════════════════" | tee -a "$LOG"
echo "" | tee -a "$LOG"
echo "  Run                         MT       CI       F8" | tee -a "$LOG"
echo "  tfm_base (baseline)         27.0mm    5.0mm    6.5mm" | tee -a "$LOG"

for name in tfm_v2d_mlpproj_300k tfm_v2e_reactive_300k tfm_v2f_attnpool_300k tfm_v2_all_300k; do
    f="results/eval/v2_probes/$name/ablation.json"
    [ -f "$f" ] || continue
